...
const DashboardHeader = ({
  ...
  const { notifications, searchJobs } = useDashboard();
                  ...
                  <ol className="flex items-center space-x-2">
                    {breadcrumbs.map((crumb, index) => (
                      <li key={index} className="flex items-center">
                        ...
                        <span className={`text-sm ${index === breadcrumbs.length - 1 ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 cursor-pointer'}`}>
                          {crumb.name}
                        </span>
                      </li>
                    ))}
