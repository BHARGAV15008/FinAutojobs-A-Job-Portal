...
const EnhancedCandidatesTab = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        console.log("🔍 [EnhancedApplicantsTab] Fetching applications...");

        const response = await applicationService.getUserApplications(50, 1);
        console.log("📊 [EnhancedApplicantsTab] Raw response:", response);
        console.log(
          "📊 [EnhancedApplicantsTab] Applications count:",
          response.data?.applications?.length
        );

        if (response.data?.applications?.length > 0) {
          console.log(
            "📊 [EnhancedApplicantsTab] First application full data:",
            response.data.applications[0]
          );
          console.log(
            "📊 [EnhancedApplicantsTab] First application applicantSnapshot:",
            response.data.applications[0].applicantSnapshot
          );
          console.log(
            "📊 [EnhancedApplicantsTab] First application applicationData:",
            response.data.applications[0].applicationData
          );
        }
  ...
  const candidates = useMemo(() => {
    return applications.map((app) => ({
      ...
      summary:
        ...
        app.applicantSnapshot?.professionalSummary ||
        "",
      qualification:
        app.applicantSnapshot?.qualification ||
        app.applicationData?.qualification ||
        "Not specified",
      currentRole:
        app.applicantSnapshot?.currentJobTitle ||
        app.applicationData?.currentJobTitle ||
        app.applicantSnapshot?.currentRole ||
        "Not specified",
      currentJobTitle:
        app.applicantSnapshot?.currentJobTitle ||
        app.applicationData?.currentJobTitle ||
        "Not specified",
      expectedSalary:
        app.applicantSnapshot?.expectedSalary ||
      ...
      experience: (() => {
        ...
        const professionalTotalExp =
          app.applicationData?.professionalInfo?.totalExperience;
        const professionalWorkExp =
          app.applicationData?.professionalInfo?.workExperience;
        const professionalExpLevel =
          app.applicationData?.professionalInfo?.experienceLevel;
        ...
        console.log(
          "🔍 Experience data for",
          app.applicantSnapshot?.fullName || "Unknown",
          {
            snapshotExperience,
            snapshotYears,
            appExperience,
        ...
        if (snapshotExperience && snapshotExperience.trim()) {
          ...
          professionalTotalExp !== undefined &&
          professionalTotalExp !== null
        ) {
      ...
      skills: (() => {
        ...
        console.log(`🔍 [Skills] for ${app.applicantSnapshot?.fullName}:`, {
          snapshotSkills,
          appDataSkills,
          snapshotType: typeof snapshotSkills,
          isSnapshotArray: Array.isArray(snapshotSkills),
        });
        ...
        console.log(
          `✅ [Skills Result] for ${app.applicantSnapshot?.fullName}:`,
          skillsArray
        );
        return skillsArray;
      })(),
      education:
        ...
        app.applicationData?.education ||
        [],
      workExperience:
        app.applicantSnapshot?.workExperience ||
        app.applicationData?.workExperience ||
        [],
      portfolioLinks: (() => {
        ...
        if (
          app.applicantSnapshot?.linkedin_url ||
          app.applicantSnapshot?.linkedinUrl
        ) {
          links.push({
            type: "LinkedIn",
            url:
        ...
        if (
          app.applicantSnapshot?.github_url ||
          app.applicantSnapshot?.githubUrl
        ) {
          links.push({
            type: "GitHub",
            url:
        ...
        if (
          app.applicantSnapshot?.portfolio_url ||
          app.applicantSnapshot?.portfolioUrl
        ) {
          links.push({
            type: "Portfolio",
            url:
      ...
      resumeUrl: (() => {
        ...
        if (
          app.applicantSnapshot?.fullName === "john wick" ||
          app.candidateName === "john wick"
        ) {
  ...
  const statusConfig = {
    ...
    interviewed: {
      ...
      count: candidates.filter((c) => c.status === "interviewed").length,
    },
    rejected: {
      label: "Rejected",
  ...
  const handleCandidateAction = async (action, candidate) => {
    ...
    try {
      ...
      switch (action) {
        case "contact":
          alert(
            `Contacting ${candidate.name} - This would send an email/message`
          );
          break;
        case "schedule":
          alert(
            `Scheduling interview with ${candidate.name} - This would open calendar`
          );
          break;
        case "shortlist":
          const handleShortlistCandidate = async (candidate) => {
            ...
            try {
              ...
              alert(
                `${candidate.name} has been ${
                  candidate.isShortlisted ? "removed from" : "added to"
                } shortlist!`
              );
            } catch (error) {
        ...
        case "reject":
          if (
            window.confirm(`Are you sure you want to reject ${candidate.name}?`)
          ) {
  ...
  const handleStatusChange = async (applicationId, newStatus) => {
    try {
      ...
      alert(
        `✅ Candidate status updated to ${newStatus} - Database updated successfully! Notification sent to candidate.`
      );
    } catch (error) {
    ...
    try {
      ...
      if (!resumeUrl && candidate.candidateId) {
        ...
        try {
          ...
          const result = await candidatesAPI.downloadResume(
        ...
        try {
          const apiUrl =
            import.meta.env.VITE_API_URL || "http://192.168.41.134:5000/api";
          ...
          if (profileData) {
            ...
            for (const field of possibleResumeFields) {
              const fieldValue = field
                .split(".")
                .reduce((obj, key) => obj?.[key], profileData);
              if (
                fieldValue &&
      ...
      if (resumeUrl) {
        ...
        try {
          ...
          console.log(
            "⚠️ Direct fetch failed, trying link method:",
            fetchError.message
          );
        }
  ...
  const handleScheduleNewInterview = async (interviewData) => {
    try {
      ...
      const formattedData = {
        ...
        title:
          interviewData.title || `Interview for ${interviewData.candidateId}`,
      ...
      if (candidateId) {
        await sendNotificationToApplicant(candidateId, {
          ...
          message: `Your interview for ${
            interviewData.jobTitle || "the position"
          } has been scheduled for ${new Date(
            formattedData.scheduledDate
          ).toLocaleDateString()} at ${formattedData.scheduledTime}`,
        ...
        alert(
          "✅ Interview scheduled successfully! (Note: Could not send notification - no candidate ID found)"
        );
      }
    } catch (error) {
    ...
    try {
      ...
      const updateData = {
        ...
        scheduledDate: newDateTime.datetime
          ...
          : new Date(
              newDateTime.date || newDateTime.scheduledDate
            ).toISOString(),
      ...
      const response = await interviewsAPI.updateInterview(
        interviewId,
        updateData
      );
      ...
      const candidateId =
        newDateTime.candidateId ||
        newDateTime.applicantId ||
        response.data?.candidateId;
      console.log(
        "🔍 Sending reschedule notification to candidate ID:",
        candidateId
      );
      await sendNotificationToApplicant(candidateId, {
        type: "interview_rescheduled",
        title: "Interview Rescheduled",
        message: `Your interview has been rescheduled to ${new Date(
          updateData.scheduledDate
        ).toLocaleDateString()} at ${updateData.scheduledTime}`,
      ...
      alert(
        "✅ Interview rescheduled successfully! Notification sent to candidate."
      );
    } catch (error) {
  ...
  const handleCancelInterview = async (interviewId) => {
    try {
      ...
      const candidateId =
        response.data?.candidateId || response.data?.applicantId;
      console.log(
        "🔍 Sending cancel notification to candidate ID:",
        candidateId
      );
      await sendNotificationToApplicant(candidateId, {
      ...
      alert(
        "✅ Interview cancelled successfully! Notification sent to candidate."
      );
    } catch (error) {
  ...
  return (
    ...
    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <motion.button
                    className={`px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors duration-200 text-sm ${
                      actionLoading[`${candidate.id}_download`]
                        ? "opacity-50"
                        : ""
                    }`}
                    whileHover={{ scale: 1.02 }}
      ...
      <div className="text-center text-gray-600 dark:text-gray-400">
        Showing {filteredAndSortedCandidates.length} of {candidates.length}{" "}
        candidates
      </div>
