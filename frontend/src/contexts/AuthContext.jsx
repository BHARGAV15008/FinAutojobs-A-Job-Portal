...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      console.log(
        "🔍 AuthContext loadUser called, token:",
        ...
        try {
          ...
          while (retryCount < maxRetries) {
            try {
              ...
              retryCount++;
              console.log(
                `🔄 Token validation retry ${retryCount}/${maxRetries}`
              );
      ...
      if (error.response) {
        ...
        switch (status) {
          ...
          default:
            errorMessage =
              serverMessage ||
              `Login failed (Error ${status}). Please try again.`;
        }
    ...
    try {
      ...
      if (error.code === "ECONNABORTED" || error.message.includes("timeout")) {
        ...
        console.log(
          "📧 Mock OTP for",
          email,
          ": Use any 6-digit code (backend validation disabled in mock mode)"
        );
...
export const useAuth = () => {
  ...
  if (context === null) {
    console.warn(
      "AuthContext is null, this might be during initialization or hot reload"
    );
    return null;
  }
  return context;
