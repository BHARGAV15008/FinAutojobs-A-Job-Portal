...
import NaukriFooter from "../components/layout/NaukriFooter";
...
import NaukriJobsPage from "../pages/NaukriJobsPage";
import NaukriCompaniesPage from "../pages/NaukriCompaniesPage";
import NaukriHomePage from "../pages/NaukriHomePage";
...
const SalaryInsightsPage = React.lazy(() =>
  import("../pages/SalaryInsightsPage").catch(() => ({
    default: () => <div>Salary Insights Page Loading...</div>,
  }))
);

const SkillsAssessmentPage = React.lazy(() =>
  import("../pages/SkillsAssessmentPage").catch(() => ({
...
const JobAlertsPage = React.lazy(() =>
  import("../pages/JobAlertsPage").catch(() => ({
    default: () => <div>Job Alerts Page Loading...</div>,
  }))
);

const AddJobPage = React.lazy(() =>
  import("../pages/AddJobPage").catch(() => ({
...
const AppRoutes = () => {
  ...
  // Use Naukri-style UI for main pages (set to true to enable)
  const useNaukriStyle = true;
  ...
  return (
    ...
    >
      ...
      {/* Navigation - Use Naukri style or original */}
      {!isDashboard &&
        !isAdminLogin &&
        (useNaukriStyle ? <NaukriNavbar /> : <Navigation />)}
      <Box
        component="main"
        sx={{ flex: 1, width: "100%", overflow: "visible" }}
      >
        <React.Suspense
          fallback={
            <SmoothLoader loading={true} message="Loading page..." size={50} />
          }
        >
          <Switch>
            {/* Public Routes - Use Naukri-style components */}
            <Route path="/">
              {() => (useNaukriStyle ? <NaukriHomePage /> : <HomePageNew />)}
            </Route>
            <Route path="/jobs">
              {() => (useNaukriStyle ? <NaukriJobsPage /> : <JobsPage />)}
            </Route>
            <Route path="/companies">
              {() =>
                useNaukriStyle ? <NaukriCompaniesPage /> : <CompaniesPage />
              }
            </Route>
            ...
            <Route path="/oauth/link-callback">
              {() => <OAuthLinkCallback />}
            </Route>
      ...
      {/* Footer - Hide on dashboard and auth pages, use Naukri style */}
      {!isDashboard &&
        !isAuthPage &&
        (useNaukriStyle ? (
          <NaukriFooter />
          ...
          <Box
            component="footer"
            sx={{ bgcolor: "grey.900", color: "white", py: 4, mt: "auto" }}
          >
            <Box
              ...
              >
                ...
                >
                  <Box>
                    ...
                    >
                      India's leading specialized job platform connecting top
                      talent with premier opportunities in Finance and
                      Automotive sectors.
                    </Box>
                  </Box>
                  <Box>
                    ...
                    <Box
                      component="ul"
                      sx={{ listStyle: "none", p: 0, "& li": { mb: 0.5 } }}
                    >
                      <li>
                        <Box
                          component="a"
                ...
                >
                  <Box component="p" sx={{ fontSize: "0.8rem", m: 0 }}>
                    &copy; 2025 FinAutoJobs. All rights reserved.
                  </Box>
                </Box>
              </Box>
            </Box>
