...
const JobsPage = () => {
    ...
    if (user && (user.id || user._id)) {
      ...
      try {
        ...
        if (savedFavorites) {
          ...
          console.log(
            `Loaded ${favoritesArray.length} favorites from localStorage`
          );
        }
        if (savedBookmarks) {
          ...
          console.log(
            `Loaded ${bookmarksArray.length} bookmarks from localStorage`
          );
        }
      } catch (error) {
    ...
    if (user && (user.id || user._id)) {
      ...
      if (pendingApplication) {
        try {
          ...
          const isRecent =
            Date.now() - applicationData.timestamp < 10 * 60 * 1000;

          if (isRecent && applicationData.jobId) {
            console.log(
              "Found pending job application, looking for job",
              applicationData.jobTitle
            );
            // Find the job in current jobs list
            const job = jobs.find(
              (j) => (j.id || j._id) === applicationData.jobId
            );
            if (job) {
              console.log(
                "Found job for pending application, opening application modal"
              );
              setSelectedJobForApplication(job);
              setApplicationModalOpen(true);
            } else {
              console.warn(
                "Job not found in current list, user can manually apply"
              );
            }
  ...
  const toggleBookmark = (jobId) => {
    ...
    try {
      ...
      localStorage.setItem(
        `bookmarks_${userId}`,
        JSON.stringify([...newLocalBookmarks])
      );
      ...
      if (user && (user.id || user._id)) {
        try {
          ...
          const appliedJobIds = new Set(
            applicationsData.map((app) => app.jobId || app.job_id || app.id) ||
              []
          );
          ...
          console.log(
            "ℹ️ No applications found (this is normal for new users)"
          );
  ...
  const filteredJobs = (Array.isArray(jobs) ? jobs : []).filter((job) => {
    ...
    const matchesWorkMode =
      !selectedWorkMode ||
      job.workMode === selectedWorkMode ||
      (selectedWorkMode === "Remote" && job.remote);
    const matchesCompanyType =
      !selectedCompanyType || job.companyType === selectedCompanyType;
    const matchesEducation =
      !selectedEducation || job.education === selectedEducation;
    ...
    if (selectedDatePosted && selectedDatePosted !== "Anytime") {
      ...
      if (selectedDatePosted === "Last 24 hours")
        matchesDatePosted = diffDays <= 1;
      else if (selectedDatePosted === "Last 7 days")
        matchesDatePosted = diffDays <= 7;
      else if (selectedDatePosted === "Last 15 days")
        matchesDatePosted = diffDays <= 15;
      else if (selectedDatePosted === "Last 30 days")
        matchesDatePosted = diffDays <= 30;
    }
    ...
    if (selectedSalaryRange) {
      ...
      matchesSalaryRange = salary.includes(
        selectedSalaryRange.replace("₹", "").split("-")[0]
      );
    }
    ...
    if (selectedTab === 1)
      return matchesAllFilters && job.department === "Finance";
    if (selectedTab === 2)
      return matchesAllFilters && job.department === "Engineering";
    ...
    <JobCard onClick={() => handleViewDetails(job)}>
      ...
      <Box sx={{ flexGrow: 1, minWidth: 0 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", mb: 1.25 }}>
          <CurrencyRupee
            sx={{ fontSize: 15, color: "text.secondary", mr: 0.25 }}
          />
          <Typography
            variant="body2"
            color="text.secondary"
            sx={{ fontSize: "0.8rem" }}
          >
            {formatSalaryDisplay(job.salaryMin, job.salaryMax, job.salary) ||
              "Not disclosed"}
          </Typography>
        </Box>
              ...
              <Paper sx={{ p: 6, textAlign: "center", mb: 4 }}>
                <Box sx={{ mb: 3 }}>
                  ...
                  <Typography variant="body1" color="text.secondary" paragraph>
                    ...
                    adjusting your search filters or check back later for new
                    opportunities.
                  </Typography>
                  <Box
                        ...
                        <TableCell>
                          <Box sx={{ display: "flex", gap: 1 }}>
                            ...
                            <Button
                              ...
                              sx={{
                                color: appliedJobs.has(job.id)
                                  ? "success.main"
                                  : undefined,
                                borderColor: appliedJobs.has(job.id)
                                  ? "success.main"
                                  : undefined,
                              }}
                            >
            ...
            {filteredJobs.length === 0 && (
              <Paper sx={{ p: 6, textAlign: "center" }}>
                ...
                <Typography variant="body2" color="text.secondary" paragraph>
                  No jobs have been posted by recruiters yet. Try adjusting your
            ...
            <Grid item xs={12} md={4}>
              <Alert severity="info">
                ...
                <Typography variant="body2">
                  Finance jobs increased by 15% this quarter with high demand
                  for analysts and risk managers.
                </Typography>
              </Alert>
            </Grid>
            <Grid item xs={12} md={4}>
              <Alert severity="success">
                ...
                <Typography variant="body2">
                  Electric vehicle sector is booming with 25% growth in
          ...
          <DialogContent>
            {viewDetailsModal.job && (
              <Box sx={{ pt: 1 }}>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  ...
                  <Grid item xs={6}>
                    ...
                    >
                      ...
                      <Typography variant="body2" fontWeight="bold">
                        {viewDetailsModal.job.experience}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={6}>
                    ...
                    >
                      ...
                      <Typography variant="body2" fontWeight="bold">
                        {viewDetailsModal.job.workMode || "Onsite"}
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
                ...
                {/* Skills Required */}
                {viewDetailsModal.job.skills &&
                  viewDetailsModal.job.skills.length > 0 && (
                    <Box sx={{ mb: 2.5 }}>
                      <Typography
                        variant="subtitle2"
                        fontWeight="bold"
                ...
                {/* Benefits */}
                {viewDetailsModal.job.benefits &&
                  viewDetailsModal.job.benefits.length > 0 && (
                    <Box sx={{ mb: 2.5 }}>
                      <Typography
                        variant="subtitle2"
                        fontWeight="bold"
                ...
                >
                  ...
                  <Grid container spacing={1.5}>
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body2" fontWeight="500">
                        {viewDetailsModal.job.industry || "Not specified"}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography
                      ...
                      <Typography variant="body2" fontWeight="500">
                        {viewDetailsModal.job.companySize || "Not specified"}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography
                      ...
                      <Typography variant="body2" fontWeight="500">
                        {viewDetailsModal.job.posted || "Recently"}
                      </Typography>
                    </Grid>
                  </Grid>
                </Box>
                ...
                <Box sx={{ display: "flex", gap: 3, mb: 2.5 }}>
                  <Box sx={{ textAlign: "center" }}>
                    <Typography
                      ...
                      fontWeight="bold"
                    >
                      {viewDetailsModal.job.views || 150}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      Views
                    </Typography>
                  ...
                  <Box sx={{ textAlign: "center" }}>
                    <Typography
                      ...
                      fontWeight="bold"
                    >
                      {viewDetailsModal.job.applicants || 0}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      Applicants
                    </Typography>
                ...
                <Box sx={{ display: "flex", gap: 0.75, flexWrap: "wrap" }}>
                  {viewDetailsModal.job.featured && (
                    <Chip label="Featured" color="primary" size="small" />
                  )}
                  {viewDetailsModal.job.urgentHiring && (
                    <Chip label="Urgent Hiring" color="error" size="small" />
                  )}
                  {viewDetailsModal.job.remote && (
                    <Chip label="Remote" color="success" size="small" />
                  )}
                  {viewDetailsModal.job.verified && (
                    <Chip label="Verified Company" color="info" size="small" />
                  )}
                </Box>
              </Box>
            )}
