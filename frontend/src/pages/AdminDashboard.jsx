...
import {
  ...
  Person, Work, Analytics, Dashboard, AdminPanelSettings, Security,
  Database, Shield, Report
} from 'lucide-react';
import {
  Container,
  Box,
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const handleNewJobPosted = (data) => {
      // Update recent activity with new job posting
    ...
    const handleApplicationStatusUpdated = (data) => {
      // Update recent activity with application status change
      setRecentActivity(prev => [
        {
          id: Date.now(),
    ...
    const handleNewMessage = (data) => {
      ...
      toast({
        title: "New Message",
        description: `New message from ${data.senderName}`,
        variant: "default"
      });
    };

    // Add WebSocket event listeners
    const cleanupNewJob = addEventListener('new_job_posted_admin', handleNewJobPosted);
    const cleanupApplication = addEventListener('application_status_updated', handleApplicationStatusUpdated);
    const cleanupMessage = addEventListener('new_message', handleNewMessage);
  ...
  const CompanyRow = ({ company }) => (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${company.verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
            }`}>
  ...
  const userData = [
    { name: 'Applicants', value: users.filter(u => u.role === 'applicant').length, color: '#8884d8' },
    { name: 'Recruiters', value: users.filter(u => u.role === 'employer').length, color: '#82ca9d' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <Typography variant="h6" fontWeight="bold" className="text-purple-600">
  ...
  return (
    <ModernDashboardLayout title="Admin Dashboard" userRole="admin">
