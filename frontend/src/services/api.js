...
export const authAPI = {
  ...
  // Test POST endpoint
  testPostEndpoint: (testData) => api.post("/auth/test-frontend", testData),
  // Test PUT endpoint
  testPutEndpoint: (testData) => api.put("/auth/test-put", testData),
  checkAvailability: (field, value, role) =>
    api.post("/auth/check-availability", { field, value, role }),
  generateUsername: (firstName, lastName, role) =>
    api.post("/auth/generate-username", { firstName, lastName, role }),
  validateUsername: (username) =>
    api.post("/auth/validate-username", { username }),
  sendOTPEmail: (email) =>
    api.post("/auth/send-otp-email", { email }, { timeout: 60000 }),
  sendOTPSMS: (phone) =>
    api.post("/auth/send-otp-sms", { phone }, { timeout: 60000 }),
  verifyOTP: (identifier, otp, type) =>
    api.post("/auth/verify-otp", { identifier, otp, type }),
...
export const candidatesAPI = {
  ...
  sendEmail: (id, emailData) =>
    api.post(`/candidates/${id}/send-email`, emailData),
  ...
  bulkUpdate: (candidateIds, updateData) =>
    api.put("/candidates/bulk-update", { candidateIds, ...updateData }),
  searchCandidates: (query, filters) =>
    api.post("/candidates/search", { query, filters }),
  getCandidateNotes: (id) => api.get(`/candidates/${id}/notes`),
  addCandidateNote: (id, note) => api.post(`/candidates/${id}/notes`, { note }),
  updateCandidateNote: (id, noteId, note) =>
    api.put(`/candidates/${id}/notes/${noteId}`, { note }),
  deleteCandidateNote: (id, noteId) =>
    api.delete(`/candidates/${id}/notes/${noteId}`),
};
...
export const usersAPI = {
  ...
  exportUsers: (filters) =>
    api.post("/users/export", filters, { responseType: "blob" }),
};
...
export const interviewsAPI = {
  ...
  addFeedback: (id, feedbackData) =>
    api.put(`/interviews/${id}/feedback`, feedbackData),

  // Get interviews by candidate
  getInterviewsByCandidate: (candidateId) =>
    api.get(`/interviews?candidateId=${candidateId}`),
...
export const analyticsAPI = {
  getDashboardAnalytics: (role) =>
    api.get(`/analytics/dashboard/${role || ""}`),
  getPlatformAnalytics: (period) =>
    api.get(`/analytics/platform?period=${period}`),
  getUserAnalytics: (userId, period) =>
    api.get(`/analytics/user/${userId}?period=${period}`),
  getJobAnalytics: (jobId, period) =>
    api.get(`/analytics/job/${jobId}?period=${period}`),
  getCompanyAnalytics: (companyId, period) =>
    api.get(`/analytics/company/${companyId}?period=${period}`),
  ...
  getSalaryInsights: (filters) =>
    api.post("/analytics/salary-insights", filters),
};
...
export const settingsAPI = {
  ...
  updateEmailTemplate: (id, template) =>
    api.put(`/settings/email-templates/${id}`, template),
};
...
export const searchAPI = {
  ...
  searchCandidates: (query, filters) =>
    api.post("/search/candidates", { query, filters }),
  searchCompanies: (query, filters) =>
    api.post("/search/companies", { query, filters }),
  getSearchSuggestions: (query, type) =>
    api.get(`/search/suggestions?q=${query}&type=${type}`),
};
...
export const messagesAPI = {
  ...
  getConversation: (userId, limit = 50) =>
    api.get(`/messages/conversation/${userId}?limit=${limit}`),
...
export const adminAPI = {
  ...
  getSystemAnalytics: (params) =>
    api.get("/admin/analytics/system", { params }),

  // Content Moderation
  getModerationItems: (params) =>
    api.get("/admin/moderation/items", { params }),
  approveModerationItem: (id, data) =>
    api.post(`/admin/moderation/${id}/approve`, data),
  rejectModerationItem: (id, data) =>
    api.post(`/admin/moderation/${id}/reject`, data),
