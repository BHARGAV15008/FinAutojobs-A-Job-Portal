...
async function testEndpoint(name, endpoint, options = {}) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      log.success(
        `${name} - ${test.responseTime}ms - Status: ${response.status}`
      );
      ...
      if (options.validator) {
        ...
        if (!validationResult.valid) {
          ...
          log.warning(
            `${name} - Response validation warning: ${validationResult.error}`
          );
        }
      }
    } else {
...
async function runTests() {
  ...
  await testEndpoint("Get All Jobs", "/jobs?limit=10", {
    validator: (data) => {
      if (!data.success)
        return { valid: false, error: "Success flag is false" };
      ...
      if (!Array.isArray(data.data.jobs))
        return { valid: false, error: "Jobs is not an array" };
      return { valid: true };
    },
  });
  ...
  await testEndpoint("Get All Companies", "/companies?limit=10", {
    validator: (data) => {
