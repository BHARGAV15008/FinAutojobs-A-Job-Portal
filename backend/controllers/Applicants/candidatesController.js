...
export const getCandidates = async (req, res) => {
  try {
    ...
    if (experience_min) {
      whereConditions.push(
        sql`${users.experience_years} >= ${parseInt(experience_min)}`
      );
    }

    if (experience_max) {
      whereConditions.push(
        sql`${users.experience_years} <= ${parseInt(experience_max)}`
      );
    }
    ...
    // Build order by
    const orderBy =
      sort_order === "asc" ? asc(users[sort_by]) : desc(users[sort_by]);
...
export const sendEmailToCandidate = async (req, res) => {
  try {
    const { id } = req.params;
    const { subject, message, template, schedule_send, schedule_date } =
      req.body;
...
export const downloadCandidateResume = async (req, res) => {
  try {
    ...
    if (candidate.resume_url.startsWith("http")) {
      ...
      // Relative path from uploads - use project root
      resumePath = path.join(projectRoot, candidate.resume_url.substring(1));
    } else if (candidate.resume_url.includes("uploads/")) {
      // Path includes uploads
      resumePath = path.join(projectRoot, candidate.resume_url);
    } else {
...
export const getCandidateStats = async (req, res) => {
  try {
    ...
    const recentCandidatesResult = await db
      .select({ count: sql`COUNT(*)` })
      .from(users)
      .where(
        and(
    ...
    const candidatesByExperienceResult = await db
      .select({
      ...
      .from(users)
      .where(eq(users.role, "applicant")).groupBy(sql`
      CASE 
        WHEN ${users.experience_years} <= 2 THEN 'Entry Level (0-2 years)'
        WHEN ${users.experience_years} <= 5 THEN 'Mid Level (3-5 years)'
        WHEN ${users.experience_years} <= 10 THEN 'Senior Level (6-10 years)'
    ...
    const updatedCandidates = await db
      ...
      .where(
        and(
          inArray(
            users.id,
            candidate_ids.map((id) => parseInt(id))
          ),
          eq(users.role, "applicant")
        )
      )
