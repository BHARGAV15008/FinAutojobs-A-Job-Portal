import {
  ...
  Admin,
  getUserModel,
} from "../models/mongodb/UserSchemas.js";
...
export const getProfile = async (req, res) => {
  try {
    ...
    // Parse JSON string fields if they exist (fix for stringified objects)
    if (user && user.toObject) {
      const userObject = user.toObject();

      // Parse documents field if it's a string
      if (userObject.documents && typeof userObject.documents === "string") {
        try {
          userObject.documents = JSON.parse(userObject.documents);
        } catch (e) {
          console.error("Failed to parse documents:", e);
        }
      }

      // Parse careerInfo field if it's a string
      if (userObject.careerInfo && typeof userObject.careerInfo === "string") {
        try {
          userObject.careerInfo = JSON.parse(userObject.careerInfo);
        } catch (e) {
          console.error("Failed to parse careerInfo:", e);
        }
      }

      // Parse jobPreferences field if it's a string
      if (
        userObject.jobPreferences &&
        typeof userObject.jobPreferences === "string"
      ) {
        try {
          userObject.jobPreferences = JSON.parse(userObject.jobPreferences);
        } catch (e) {
          console.error("Failed to parse jobPreferences:", e);
        }
      }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const updatedUser = await UserModel.findByIdAndUpdate(userId, cleanedData, {
      new: true,
      runValidators: true,
    }).select("-password");
