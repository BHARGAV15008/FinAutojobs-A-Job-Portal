...
export const register = async (req, res) => {
  try {
    let { username, email, password, fullName, phone, role = 'applicant' } = req.body;
    ...
    // Smart username generation if not provided or if duplicate
    if (!username || !fullName) {
      return res.status(400).json({
        message: 'Full name is required for username generation',
        field: 'fullName',
        status: 'error'
      });
    }
    ...
    const validation = UsernameGenerator.validateUsername(finalUsername);
    if (!validation.valid) {
      return res.status(400).json({
...
export const login = async (req, res) => {
  try {
    ...
    const result = await db.select().from(users).where(or(eq(users.email, email), eq(users.username, email)));
...
;// Remove redundant authenticateToken declaration// Middleware to authenticate JWT token
