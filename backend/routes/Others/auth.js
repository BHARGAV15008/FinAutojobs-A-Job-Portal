...
import {
  ...
  sendSMSOTP,
  verifyOTPCode,
} from "../../Controllers/Others/otpController.js";
...
router.get("/profile", authenticateToken, async (req, res) => {
  try {
    ...
    const userData = {
      ...
      profileCompleted:
        user.profileCompleted || user.profile_completed || false,
    ...
    try {
      ...
      if (user.active_sessions && user.active_sessions.length > 0) {
        return res.status(409).json({
          success: false,
          message:
            "User is already logged in from another device. Please logout first.",
  ...
  [
    ...
    body().custom((value, { req }) => {
      ...
      if (!full_name && (!firstName || !lastName)) {
        throw new Error(
          "Either full_name or both firstName and lastName must be provided"
        );
      }
      return true;
    }),
    ...
    body("role")
      ...
      .withMessage(
        "Role must be either job_seeker, jobseeker, employer, recruiter, or admin"
      ),
    body("phone")
      .optional()
      .matches(/^[0-9+\-\s()]+$/)
...
router.put("/profile", authenticateToken, async (req, res) => {
  try {
    ...
    // Parse JSON strings in updateData if they exist
    ...
    if (updateData.careerInfo && typeof updateData.careerInfo === "string") {
      try {
        updateData.careerInfo = JSON.parse(updateData.careerInfo);
      } catch (e) {
    ...
    if (
      ...
      typeof updateData.jobPreferences === "string"
    ) {
      try {
        updateData.jobPreferences = JSON.parse(updateData.jobPreferences);
      } catch (e) {
