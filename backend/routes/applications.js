...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (!process.env.JWT_SECRET) {
      console.warn(
        "⚠️ Using default JWT secret - configure JWT_SECRET in environment variables for production"
      );
    }
    ...
    console.log(
      "🔍 JWT decoded successfully, looking up user:",
      decoded.userId
    );
    ...
    if (!user && decoded.email) {
      try {
        ...
        console.log(
          "🔍 BaseUser lookup by email result:",
          user ? "Found" : "Not found"
        );
      } catch (error) {
    ...
    if (!user && decoded.username) {
      try {
        ...
        console.log(
          "🔍 BaseUser lookup by username result:",
          user ? "Found" : "Not found"
        );
      } catch (error) {
    ...
    if (!user && decoded.id) {
      try {
        ...
        console.log(
          "🔍 BaseUser lookup by alternative id result:",
          user ? "Found" : "Not found"
        );
      } catch (error) {
        console.log(
          "⚠️ BaseUser lookup by alternative id failed:",
          error.message
        );
      }
    }
    ...
    if (!user) {
      try {
        ...
        console.log(
          "🔍 CleanUser fallback lookup result:",
          user ? "Found" : "Not found"
        );
        if (!user && decoded.email) {
          user = await CleanUser.findOne({ email: decoded.email });
          console.log(
            "🔍 CleanUser fallback by email result:",
            user ? "Found" : "Not found"
          );
        }
      } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      const applicationData = {
        ...
        // Skills
        skills: {
          primary: req.body.primarySkills
            ? req.body.primarySkills.split(",").map((s) => s.trim())
            : [],
          technical: req.body.technicalSkills
            ? req.body.technicalSkills.split(",").map((s) => s.trim())
            : [],
          soft: req.body.softSkills
            ? req.body.softSkills.split(",").map((s) => s.trim())
            : [],
        },
...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === "applicant") {
      ...
      const userJobs = await Job.find({ postedBy: req.user.userId }).select(
        "_id"
      );
    ...
    const [applications, totalCount] = await Promise.all([
      Application.find(query)
        ...
        .populate({
          path: "applicantId",
          select:
            "firstName lastName email phone profileImage bio skills education workExperience socialLinks portfolioLinks documents",
          model: "BaseUser",
        })
        .populate({
          path: "applicationInfo",
          strictPopulate: false,
        }) // Populate ApplicationInformation
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
    ...
    console.log(
      ...
      applications[0]
        ? {
            ...
            applicationInfoId:
              applications[0].applicationInfo?._id ||
              applications[0].applicationInfo,
            applicationInfoKeys: applications[0].applicationInfo
              ? Object.keys(applications[0].applicationInfo)
              : [],
          }
        : "No applications found"
    );

    // Debug: Check if applicationInfo has actual data
    if (applications[0]?.applicationInfo) {
      console.log("✅ ApplicationInfo populated with data:");
      console.log("   - basicInfo:", applications[0].applicationInfo.basicInfo);
      console.log(
        "   - experience:",
        applications[0].applicationInfo.experience
      );
    } else {
      console.log("❌ ApplicationInfo is null/undefined");
    }
    ...
    const transformedApplications = applications.map((application) => ({
      ...
      jobSnapshot: {
        title:
          ...
          application.jobSnapshot?.jobTitle ||
          "Unknown Position",
        company:
        ...
        type:
          ...
          application.jobSnapshot?.jobType ||
          "Unknown Type",
      },
    ...
    console.log(
      "🔍 Transformed applications sample:",
  ...
  try {
    ...
    const canView =
      ...
      (req.user.role === "recruiter" &&
        req.user.userId === application.recruiterId?.toString());
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "🔍 BaseUser lookup result:",
        applicantUser ? "Found" : "Not found"
      );

      if (!applicantUser) {
        console.log(
          "❌ Applicant user not found in BaseUser for ID:",
          req.user.userId
        );
      ...
      const applicantSnapshot = {
        ...
        location:
          req.body.location ||
          applicantUser.currentLocation?.city ||
          applicantUser.location ||
          "",
        currentJobTitle:
          req.body.currentJobTitle ||
          applicantUser.careerInfo?.currentJobTitle ||
          "",
        currentCompany:
          req.body.currentCompany ||
          applicantUser.careerInfo?.currentCompany ||
          "",
        experience:
      ...
      // Create ApplicationInformation for complete data snapshot
      console.log("🔍 Creating ApplicationInformation document...");
      let applicationInfoCreated = false;
      try {
        // Helper function to convert string array to skill objects
        ...
        // Parse skills if they're stringified
        let userSkills = applicantUser.skills || [];
        if (typeof userSkills === "string") {
          try {
            userSkills = JSON.parse(userSkills);
          } catch (e) {
            console.warn("⚠️ Failed to parse user skills, using empty array");
            userSkills = [];
          }
        }
        if (
        ...
        const applicationInfo = new ApplicationInformation({
          ...
          expectedSalary: {
            rawSalaryText: req.body.expectedSalary || "",
            displayText: req.body.expectedSalary || "",
          },
          experience: {
            totalYears: applicantUser.yearsOfExperience || 0,
            rawExperienceText:
              req.body.experience ||
              `${applicantUser.yearsOfExperience || 0} years`,
            currentJob: {
              jobTitle:
                req.body.currentJobTitle ||
                applicantUser.careerInfo?.currentJobTitle ||
          ...
          education: Array.isArray(applicantUser.education)
            ...
            : [],
          workExperience: Array.isArray(applicantUser.workExperience)
            ? applicantUser.workExperience.filter(
                (we) => we.jobTitle && we.companyName && we.startDate
              )
            : [],
          skills: {
          ...
          location: {
            current: {
              country:
                applicantUser.currentLocation?.country ||
                applicantUser.address?.country ||
                "",
              state:
                applicantUser.currentLocation?.state ||
                applicantUser.address?.state ||
                "",
              city:
                applicantUser.currentLocation?.city ||
                applicantUser.address?.city ||
                "",
            },
            preferences: {
              willingToRelocate: req.body.willingToRelocate === "true" || false,
          ...
          jobPreferences: {
            willingToRelocate: req.body.willingToRelocate === "true" || false,
            remoteWorkPreference:
              req.body.remoteWorkPreference === "true" ? "fully-remote" : "no",
          },
          documents: {
            resumeUrl: applicationData.resumeUrl,
            coverLetterUrl: req.body.coverLetterUrl || "",
            portfolioUrl:
              req.body.portfolioUrl || applicantUser.portfolio_url || "",
          },
          socialLinks: {
            linkedin: {
              url:
                req.body.linkedinUrl ||
                applicantUser.socialLinks?.linkedinUrl ||
                applicantUser.linkedin_url ||
                "",
            },
            github: {
              url:
                applicantUser.socialLinks?.githubUrl ||
                applicantUser.github_url ||
                "",
            },
            portfolio: {
              url:
                req.body.portfolioUrl ||
                applicantUser.socialLinks?.portfolioUrl ||
                applicantUser.portfolio_url ||
                "",
            },
          },
          applicationSpecific: {
        ...
        console.log(
          "✅ ApplicationInformation created with ID:",
          savedAppInfo._id
        );

        // Link ApplicationInformation to Application
        savedApplication.applicationInfo = savedAppInfo._id;
        await savedApplication.save();
        console.log("✅ ApplicationInformation linked to Application");
        applicationInfoCreated = true;
      } catch (appInfoError) {
        console.error(
          "❌ CRITICAL: Failed to create ApplicationInformation:",
          appInfoError.message
        );
        console.error("❌ Error stack:", appInfoError.stack);
        console.error(
          "❌ Validation errors:",
          appInfoError.errors ? Object.keys(appInfoError.errors) : "None"
        );
        // Log which required fields are missing
        if (appInfoError.errors) {
          Object.keys(appInfoError.errors).forEach((field) => {
            console.error(
              `   - ${field}: ${appInfoError.errors[field].message}`
            );
          });
        }
        // Continue without failing the application, but log the issue
        console.warn(
          "⚠️ Application created without detailed ApplicationInformation"
        );
      }
      ...
      try {
        ...
        console.log(
          "⚠️ Application notification failed:",
      ...
      res.status(201).json({
        ...
        data: {
          application: {
            ...
            hasApplicationInfo: applicationInfoCreated,
          },
        },
      });
    } catch (error) {
...
router.put("/:id/status", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      ...
      updatedApplication ? "Success" : "Failed"
    );
    ...
    if (io && application.applicantId?._id) {
      io.to(`user-${application.applicantId._id}`).emit(
...
router.get("/job/:jobId", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      `🔍 Found ${applications.length} applications for job ${jobId}`
    );
...
// PUT /api/applications/:id - Update application (Applicants only)
router.put(
  "/:id",
  authenticateToken,
  upload.single("resume"),
  async (req, res) => {
    try {
      console.log("🔄 Application update request:", {
        applicationId: req.params.id,
      ...
      if (req.user.role !== "applicant") {
        console.log("❌ Access denied: User is not an applicant");
        return res.status(403).json({
          success: false,
          message: "Only applicants can update their applications",
        });
      }
      ...
      if (application.applicantId.toString() !== req.user.userId) {
        console.log("❌ Access denied: Application does not belong to user");
        return res.status(403).json({
          success: false,
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: `Cannot update application with status: ${application.applicationStatus}`,
        });
      }

      // Update resume if new file uploaded
      if (req.file) {
        const resumeUrl = `/uploads/applications/${req.file.filename}`;
        application.applicationData = application.applicationData || {};
        application.applicationData.resumeUrl = resumeUrl;
      }
      ...
      updatableFields.forEach((field) => {
        if (req.body[field] !== undefined) {
          application.applicationData = application.applicationData || {};
          application.applicationData[field] = req.body[field];
        }
      });

      // Update ApplicationInformation if it exists
      if (application.applicationInfo) {
        ...
        if (appInfo) {
          if (req.body.expectedSalary) {
            appInfo.expectedSalary.rawSalaryText = req.body.expectedSalary;
            appInfo.expectedSalary.displayText = req.body.expectedSalary;
          }
          if (req.body.coverLetter) {
            appInfo.applicationSpecific.coverLetter = req.body.coverLetter;
          }
          if (req.body.noticePeriod) {
            appInfo.applicationSpecific.noticePeriod = req.body.noticePeriod;
          }
          if (req.body.linkedinUrl) {
            appInfo.socialLinks.linkedin.url = req.body.linkedinUrl;
          }
          if (req.body.portfolioUrl) {
            appInfo.socialLinks.portfolio.url = req.body.portfolioUrl;
            appInfo.documents.portfolioUrl = req.body.portfolioUrl;
          }
          if (req.body.willingToRelocate !== undefined) {
            appInfo.location.preferences.willingToRelocate =
              req.body.willingToRelocate === "true";
          }
          if (req.body.remoteWorkPreference !== undefined) {
            appInfo.location.preferences.remoteWorkPreference =
              req.body.remoteWorkPreference === "true" ? "fully-remote" : "no";
          }
          if (req.file) {
            appInfo.documents.resumeUrl = `/uploads/applications/${req.file.filename}`;
          }
          await appInfo.save();
          console.log("✅ ApplicationInformation updated");
        }
      }
...
// DELETE /api/applications/:id - Withdraw application (Applicants only)
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    console.log("🗑️ Application withdrawal request:", {
      applicationId: req.params.id,
      userId: req.user.userId,
      role: req.user.role,
    });
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: `Cannot withdraw application with status: ${application.applicationStatus}`,
      });
    }
