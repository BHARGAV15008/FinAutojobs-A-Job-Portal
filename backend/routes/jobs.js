...
router.get("/", async (req, res) => {
  try {
    ...
    const jobsAggregation = await Job.aggregate([
      ...
      {
        $project: {
          // Project to match the existing transformedJobs structure and include applicationsCount
          _id: 1,
          jobTitle: 1,
          companyName: 1,
          location: 1,
    ...
    console.log(
      "🔍 Aggregation result (first job sample):",
      jobsAggregation[0]
    );
    ...
    const transformedJobs = jobsAggregation.map((job) => {
      ...
      const formattedSalary = (() => {
        ...
        if (salaryString && salary.period) {
          salaryString += ` ${
            salary.period.charAt(0).toUpperCase() + salary.period.slice(1)
          }`;
        }
        return salaryString;
      })();
      ...
      return {
        ...
        salaryRange: {
          // Reconstruct salaryRange object if needed by frontend
...
// POST /api/jobs/check-deadlines - Update job status based on deadline
router.post("/check-deadlines", authenticateToken, async (req, res) => {
  try {
    const now = new Date();

    // Find all active jobs with passed deadlines
