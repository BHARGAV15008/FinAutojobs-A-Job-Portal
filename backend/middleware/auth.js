...
export const verifyToken = (token) => {
  try {
    ...
    // If strict verification fails due to missing issuer/audience, try lenient verification
    if (error.name === "JsonWebTokenError") {
      try {
        console.log(
          "⚠️ Trying lenient verification without issuer/audience checks"
        );
        const decoded = jwt.verify(token, process.env.JWT_SECRET, {
          algorithms: ["HS256"],
        });
        console.log("✅ Lenient verification successful");
        return decoded;
      } catch (lenientError) {
        console.log(
          "❌ Lenient verification also failed:",
          lenientError.message
        );
        if (lenientError.name === "TokenExpiredError") {
...
export const authenticateToken = async (req, res, next) => {
  try {
    ...
    let user = await User.findById(decoded.userId).select(
      "+last_login +login_attempts +lock_until"
    );

    // Fallback: if user not found by userId, try finding by email
    if (!user && decoded.email) {
      console.log(
        "⚠️ User not found by ID, trying email lookup:",
        decoded.email
      );
      user = await User.findOne({ email: decoded.email }).select(
        "+last_login +login_attempts +lock_until"
      );
    }

    if (!user) {
      console.error("❌ USER_NOT_FOUND:", decoded.userId, decoded.email);
    ...
    if (user.status !== "active") {
      console.error("❌ INACTIVE_ACCOUNT:", {
        userId: user._id,
        status: user.status,
      });
  ...
  if (req.user && req.authMetadata) {
    try {
      ...
      if (user) {
        ...
        const existingSession = user.active_sessions.find(
...
export const authRateLimit = (maxAttempts = 5, windowMs = 15 * 60 * 1000) => {
  ...
  return (req, res, next) => {
    const key =
      req.ip + ":" + (req.body.email || req.body.username || "unknown");
    const now = Date.now();
...
export const authenticateApiKey = async (req, res, next) => {
  try {
    ...
    if (apiKey !== process.env.API_KEY) {
      console.error("INVALID_API_KEY", req, {
