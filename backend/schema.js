...
export const interviews = sqliteTable('interviews', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  job_id: integer('job_id').notNull().references(() => jobs.id, { onDelete: 'cascade' }),
  applicant_id: integer('applicant_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  type: text('type').notNull().default('video'), // 'phone', 'video', 'in-person', 'technical'
...
export const company_analytics = sqliteTable('company_analytics', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  company_id: integer('company_id').notNull().references(() => companies.id, { onDelete: 'cascade' }),
  date: text('date').notNull(), // YYYY-MM-DD format
  jobs_posted: integer('jobs_posted').notNull().default(0),
  applications_received: integer('applications_received').notNull().default(0),
  profile_views: integer('profile_views').notNull().default(0),
  interviews_scheduled: integer('interviews_scheduled').notNull().default(0),
...
export const user_preferences = sqliteTable('user_preferences', {
  ...
  font_size: text('font_size').notNull().default('medium'),
  ...
  job_alerts: integer('job_alerts', { mode: 'boolean' }).notNull().default(true),
  dashboard_layout: text('dashboard_layout').notNull().default('grid'),
...
export const user_activity_logs = sqliteTable('user_activity_logs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  user_id: integer('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
